...
try:
    observedA1,observedA2,observedB = REID_ARalgorithm_v3(loadmat("ivt_3rd_timestep.mat"),250,-90,0,1,0,370,1,2000,2)
except Exception as e:
    print(f"Module threw error when testing:{e}")
    exit()

try:
    expectedA1 = np.swapaxes(loadmat("A1.mat")["A1"], 0, 1)
    expectedA2 = np.swapaxes(loadmat("A2.mat")["A2"], 0, 1)
    expectedY = loadmat("Y.mat")["Y"]
    expectedB = loadmat("B.mat")["B"]
...
if np.array_equal(expectedB[0], observedB):
    print("B same")
else:
    print(f"B test failed\nexpected:\nobserved:{observedB}")
